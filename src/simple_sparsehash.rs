...
pub(crate) extern "C" fn sparse_array_init(element_size: u64, maximum: u32) -> *mut SparseArray {
    let mut i: u32 = 0 as u32;
    let mut arr: *mut SparseArray = 0 as *mut () as *mut SparseArray;

    /// CHECK YOUR SYSCALL RETURNS. Listen to djb.
    (arr = unsafe { calloc(1 as u64, core::mem::size_of::<SparseArray>() as u64) }
        as *mut SparseArray);
    ...
    ///do this.
    let mut stack_array: SparseArray = SparseArray {
    ...
    unsafe {
        (*arr).groups = unsafe {
            calloc(
                (unsafe { (*arr).maximum.wrapping_sub(1 as u64) } / 48 as u64)
...
extern "C" fn position_to_offset(bitmap: *const u32, position: u32) -> u32 {
    ...
    {
        '__b1: loop {
            if !(pos as u64 >= (core::mem::size_of::<u32>() as u64).wrapping_mul(8 as u64)) {
                break '__b1;
            }
            '__c1: loop {
            ...
            pos = pos
                .wrapping_sub((core::mem::size_of::<u32>() as u64).wrapping_mul(8 as u64) as u32);
        }
    }
    ...
    ///retval.
    return retval.wrapping_add(popcount_32(
        unsafe { *bitmap.add(bitmap_iter as usize) } & ((1 as u32) << pos).wrapping_sub(1),
    )) as u32;
}
...
extern "C" fn is_position_occupied(bitmap: *const u32, position: u32) -> i32 {
    return (unsafe { *bitmap.add(charbit(position as u32) as usize) } & modbit(position as u32))
        as i32;
}

extern "C" fn set_position(bitmap: *mut u32, position: u32) -> () {
    unsafe { *bitmap.add(charbit(position as u32) as usize) |= modbit(position as u32) };
}

/// Sparse Array
#[allow(unused_doc_comments)]
extern "C" fn _sparse_array_group_set(
    ...
    (offset = position_to_offset(
        &raw mut (*arr).bitmap[0 as usize] as *mut u32 as *const u32,
        i as u32,
    ));
    if (is_position_occupied(
        ...
        != 0
    {
        let to_move_siz: u64 = ((*arr).count.wrapping_sub(offset) as u64).wrapping_mul(
            (*arr)
                .elem_size
                .wrapping_add(core::mem::size_of::<u64>() as u64),
        ) as u64;
        /// Reallocate the array to hold the new item
        let new_group: *mut () = unsafe {
            realloc(
                (*arr).group,
                ((*arr).count.wrapping_add(1 as u32) as u64).wrapping_mul(
                    (*arr)
                        .elem_size
                        .wrapping_add(core::mem::size_of::<u64>() as u64),
                ),
        ...
        if to_move_siz as u64 > 0 as u64 {
            unsafe {
                __builtin___memmove_chk(
                    unsafe {
                        (new_group as *mut u8).add(
                            (offset.wrapping_add(1 as u32) as u64).wrapping_mul(
                                (*arr)
                                    .elem_size
                                    .wrapping_add(core::mem::size_of::<u64>() as u64),
                            ) as usize,
        ...
        set_position(&raw mut (*arr).bitmap[0 as usize] as *mut u32, i as u32);
    }

    /// Copy the size into the position, fighting -pedantic the whole
    ///time.
    (destination = unsafe {
        ((*arr).group as *mut u8).add(
            (offset as u64).wrapping_mul(
                (*arr)
                    .elem_size
                    .wrapping_add(core::mem::size_of::<u64>() as u64),
    ...
    ///anything.
    (destination =
        unsafe { (destination as *mut u8).add(core::mem::size_of::<u64>() as usize) } as *mut ());
    unsafe {
        __builtin___memcpy_chk(destination, val, vlen, unsafe {
            __builtin_object_size(destination as *const (), 0)
        })
    };
    return 1 as i32;
}
...
#[allow(unused_doc_comments)]
pub(crate) extern "C" fn sparse_array_set(
    arr: &SparseArray,
    i: u32,
    val: *const (),
    vlen: u64,
) -> i32 {
    if i as u64 > (*arr).maximum {
        return 0 as i32;
    }
    ...
    return _sparse_array_group_set(operating_group, position as u32, val, vlen as u64) as i32;
}

extern "C" fn _sparse_array_group_get(
    arr: &mut SparseArrayGroup,
    i: u32,
    outsize: *mut u64,
) -> *const () {
    let offset: u32 = position_to_offset(
        &raw mut (*arr).bitmap[0 as usize] as *mut u32 as *const u32,
        i as u32,
    ) as u32;
    let item_siz: *const u8 = unsafe {
        ((*arr).group as *mut u8).add(
            (offset as u64).wrapping_mul(
                (*arr)
                    .elem_size
    ...
    let item: *const () =
        unsafe { item_siz.add(core::mem::size_of::<u64>() as usize) } as *const ();
    if (is_position_occupied(
        &raw mut (*arr).bitmap[0 as usize] as *mut u32 as *const u32,
        i as u32,
    ) == 0) as i32
...
pub(crate) extern "C" fn sparse_array_get(
    arr: &SparseArray,
    i: u32,
    outsize: *mut u64,
) -> *const () {
...
pub(crate) extern "C" fn sparse_dict_init() -> *mut SparseDict {
    ...
    '__b3: loop {
        '__c3: loop {
            new = unsafe { calloc(1 as u64, core::mem::size_of::<SparseDict>() as u64) }
                as *mut SparseDict;
            ...
            unsafe {
                (*new).buckets =
                    sparse_array_init(core::mem::size_of::<SparseBucket>() as u64, 32 as u32)
            };
...
extern "C" fn _create_and_insert_new_bucket(
    ...
    '__b5: loop {
        '__c5: loop {
            ...
            copied_value = unsafe { malloc(value.len() as u64 + klen as u64) };
            if copied_value == 0 as *mut () {
                break '__b5;
            }
            unsafe {
                __builtin___memcpy_chk(
                    ...
                    value.len() as u64,
                    unsafe { __builtin_object_size(copied_value as *const (), 0) },
                )
            };
            copied_key = unsafe { copied_value.add(value.len() as u64 as usize) } as *mut i8;
            unsafe {
                __builtin___strncpy_chk(copied_key, key, klen, unsafe {
                    __builtin_object_size(copied_key as *const (), if 2 > 1 { 1 } else { 0 })
                })
            };
            let mut bct: SparseBucket = SparseBucket {
                key: copied_key,
                klen: klen,
                val: copied_value,
            ...
            if (sparse_array_set(
                ...
                core::mem::size_of::<SparseBucket>() as u64,
            ) == 0) as i32
                != 0
            {
                break '__b5;
    ...
    loop {
        ...
        '__s7: {
            match __state {
                ...
                3 => {
                    new_bucket_max = (*dict).bucket_max.wrapping_mul(2 as u64) as u64;
                    __state = 4;
                }
                4 => {
                ...
                13 => {
                    bucket =
                        sparse_array_get(unsafe { &*(*dict).buckets }, i as u32, &mut bucket_siz)
                            as *const SparseBucket;
                    __state = 14;
                }
                14 => {
                    if bucket_siz != 0 as u64 && bucket as *mut () != 0 as *mut () {
                        __state = 16;
                    } else {
                        __state = 15;
                    }
                ...
                20 => {
                    probed_val = (key_hash.wrapping_add(num_probes.wrapping_mul(num_probes) as u64)
                        & new_bucket_max.wrapping_sub(1 as u64) as u64)
                        as u32;
                    __state = 21;
                }
                21 => {
                    current_value_siz = 0 as u64;
                    __state = 22;
                }
                22 => {
                    current_value = sparse_array_get(
                        unsafe { &*new_buckets },
                        probed_val as u32,
                ...
                23 => {
                    if current_value_siz == 0 as u64 && current_value == 0 as *mut () as *const () {
                        __state = 25;
                    } else {
                        __state = 24;
                    }
...
#[allow(unused_doc_comments)]
pub(crate) extern "C" fn sparse_dict_set(
    dict: *mut SparseDict,
    key: *const i8,
    klen: u64,
    value: *const (),
    ...
    loop {
        ...
        '__s9: {
            match __state {
                ...
                12 => {
                    existing_bucket = current_value as *mut SparseBucket as *const SparseBucket;
                    __state = 15;
                }
                13 => {
                    __state = 5;
                ...
                15 => {
                    if unsafe { (*existing_bucket).hash } as u64 == key_hash
                        ...
                        && unsafe {
                            strncmp(unsafe { (*existing_bucket).key } as *const i8, key, klen)
                        } == 0
                    {
                        __state = 16;
                    } else {
                ...
                24 => {
                    unsafe {
                        printf(
                            c"Could not find an open slot in the table.\n".as_ptr() as *mut i8
                                as *const i8,
                        )
                    };
                    __state = 25;
                ...
                26 => {
                    if unsafe { (*dict).bucket_count } as f32 / unsafe { (*dict).bucket_max } as f32
                        >= 80 as f32 / 100.0
                    {
                        __state = 28;
                    } else {
...
#[allow(unused_doc_comments)]
pub(crate) extern "C" fn sparse_dict_get(
    dict: &SparseDict,
    key: *const i8,
    klen: u64,
    outsize: *mut u64,
    ...
    loop {
        let mut current_value_siz: u64 = 0 as u64;
        let probed_val: u32 = (key_hash.wrapping_add(num_probes.wrapping_mul(num_probes) as u64)
            & (*dict).bucket_max.wrapping_sub(1 as u64) as u64)
            as u32;
        let current_value: *const () = sparse_array_get(
            unsafe { &*(*dict).buckets },
            probed_val as u32,
            &mut current_value_siz,
        );
        if current_value_siz != 0 as u64 && current_value != 0 as *mut () as *const () {
            ...
            let existing_bucket: *mut SparseBucket = current_value as *mut SparseBucket;
            if unsafe { (*existing_bucket).hash } as u64 == key_hash
                && unsafe { (*existing_bucket).klen } as u64 == klen
                && unsafe { strncmp(unsafe { (*existing_bucket).key } as *const i8, key, klen) }
                    == 0
            {
                if !(outsize).is_null() {
                    unsafe {
...
pub(crate) extern "C" fn sparse_dict_free(dict: *mut SparseDict) -> i32 {
    let mut i: u32 = 0 as u32;
    {
        i = 0 as u32;
        '__b11: loop {
            if !((i as u64) < unsafe { (*dict).bucket_max }) {
                break '__b11;
            }
            '__c11: loop {
                let mut current_value_siz: u64 = 0 as u64;
